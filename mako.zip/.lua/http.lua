...
local function checkStatus(self,rsp,e1,e2,e3)
   if not rsp then return nil,e1,e2,e3 end
   local raw=self.raw
   if not self.statuscode then
      local s,e1,e2,e3=raw:status()
      if not s then return nil,e1,e2,e3 end
      if s == 301 or s == 302 then
         local r=0
         local op=self.nop
         while s == 301 or s == 302 do
...
function H:read(size) return checkStatus(self, self.raw:read(size)) end
...
function H:status(size)
   local ok,e1,e2,e3=checkStatus(self, self.raw:status(size))
   if ok then return self.statuscode end
   return nil,e1,e2,e3
end

function H:close()
   checkStatus(self,true)
...
function H:url()
   if not self.statuscode then
      local ok,e1,e2,e3=checkStatus(self,true)
      if not ok then return nil,e1,e2,e3 end
   end
