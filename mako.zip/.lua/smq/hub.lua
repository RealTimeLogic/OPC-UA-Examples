...
local function terminatePeerT(_ENV,sock,err)
   ...
   if peerT then
      ...
      if err and not peerT.closing then
...
local function onJsonMsg(ps,onmsg,data,ftid,tid,stid)
   local t = ba.json.decode(data)
   if t then
      onmsg(t,ftid,tid,stid)
   else
      prepLog(ps.env,nil,"JSON parse err for: ftid=%X, tid=%X, subtid=%X",
              ftid,tid,subtid)
      ps.onmsg(data,ftid,tid,stid)
   end
end

local function createOnJsonMsg(ps,onmsg)
   return function(data,ftid,tid,stid)
             onJsonMsg(ps,onmsg,data,ftid,tid,stid) end
end
...
local function lMsgDispatcher(sockThread, _ENV)
   while true do
      ...
      if qTail == qHead then sockThread:disable() end
   end
end
...
local function lPublish(_ENV, data, topic, subtopic)
   ...
   if not qT then -- Create queue (Ref:Q)
      ...
      if qHead == qTail then -- Empty
         ...
         qHead = (qHead + 1) % qSize
         qElems = 1
         qConsumer:enable() -- Start lMsgDispatcher coroutine
      else
